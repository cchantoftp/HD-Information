...
$diskDetailsList = foreach ($diskId in $diskIds) {
    ...
    foreach ($diskDetail in $diskDetailsList) {
        ...
        # Add missing properties as 'na'
        #Add-MissingProperties -obj $outputObj -properties $combinedProperties

        # Append the object to the CSV file
        #$outputObj | Export-Csv -Append -NoTypeInformation -Path $csvFilePath
    }
}
