...
foreach ($rg in $resourceGroups) {
    ...
    # Azure CLI command to get physical drive details
    $physicalDriveDetails = az disk list --query "[?resourceGroup=='$rg'].{Name:name, Lun:lun, DiskSize:diskSizeGb}" -o json | ConvertFrom-Json

    # Format and output the physical drive details
    $physicalDriveDetails | ForEach-Object {
        Write-Host "Name: $($_.Name), LUN: $($_.Lun), DiskSize: $($_.DiskSize)GB"
    }
}