...
foreach ($rg in $resourceGroups) {
    ...
    foreach ($vm in $windowsVMs) {
        ...
        if ($result.value) {
            ...
            $output | Export-Csv -Append -Path "windows.csv" -NoTypeInformation
        } else {
    ...
    foreach ($vm in $linuxVMs) {
        ...
        if ($result.value) {
            ...
            $output | Export-Csv -Append -Path "linux.csv" -NoTypeInformation
        } else {
            Write-Host "Skipping Linux VM '$vm' in resource group '$rg' as it is not running or set to run."
        }
    }

    # Get physical drive details for the resource group
    $physicalDriveDetails = az disk list --resource-group $rg --query "[].{Name:name, Lun:lun, DiskSize:diskSizeGb}" -o json | ConvertFrom-Json

    # Export physical drive details to CSV
    $physicalDriveDetails | Export-Csv -Path "physical_drives_$rg.csv" -NoTypeInformation
}